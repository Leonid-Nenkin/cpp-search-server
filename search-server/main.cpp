...
public:
    ...
    void AddDocument(int document_id, const string& document) {
        ...
        for (string word : words) {
            n = count(words.begin(), words.end(), word);
            n /= words.size();
    ...
    vector<Document> FindAllDocuments(const Query& query) const {
        ...
        for (string word : query.plus_words) {
            if (word_to_document_freqs_.count(word)) {
                ...
                for (const auto& i : docs) {
                    document_to_relevance[i.first] += i.second * log(double(document_count_) / word_occurence);
                }
            }
        }
        ...
        for (string word : query.minus_words) {
            if(word_to_document_freqs_.count(word)>0) {
                for(const auto& document_id : word_to_document_freqs_.at(word)){
                    document_to_relevance.erase(document_id.first);
                }
            }
        }
        
        for (auto& iter : document_to_relevance) {
            matched_documents.push_back({ iter.first, iter.second });
        }
